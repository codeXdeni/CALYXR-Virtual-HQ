...
✓ Pixel Office — autonomous agent simulation tab, rendered on `<canvas>`. A new "Office" tab (`index.html`, `scripts/pixel-office.js`) presents the HQ as a 3-floor building: click a floor band to see its rooms, click a room to walk in and see who's there. 7 rooms across 3 floors (Executive: ARIES + LIBRA; Operations: TAURUS + SAGITTARIUS + a shared Break Room; Ground: Lobby + VIRGO). No player character — agents move on their own, driven by the existing `generateCollaborations()` logic in `script.js` (an agent with an active collaboration walks to that department's room for a few ticks, then returns home). When two agents share a room it's flagged and logged to the existing Executive Activity Feed. Each agent has a job-type archetype (PM/Dev/QA/Design/Intern, see `AGENT_JOB_TYPES`) with an animated chibi sprite (idle-blink loop, switching to an executing-task animation whenever `agentStatus[name].status === "Working"` — real data, not invented state) sourced from two free CraftPix packs. Clicking an agent in its room updates a DOM info panel with live role/status/task. Resets to home rooms on page load — no persistence, since this only runs while the page is open.

The sprite frames are git-ignored (`assets/sprites/*/*.png`), not committed — CraftPix's free license forbids redistributing the raw files in a way that makes them extractable, which committing to this public repo would do. See `assets/sprites/README.md` for the license note and exact file/character mapping. Anyone else cloning the repo gets a plain colored-square fallback until they add their own art.
